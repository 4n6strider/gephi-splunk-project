...
public class NodeLabelRenderer implements Renderer {
    ...
    public void renderG2D(G2DTarget target, String label, float x, float y, int fontSize, Color color, float outlineSize, Color outlineColor, boolean showBox, Color boxColor) {
        ...
        font = font.deriveFont(fontSize*3f);
        ...
        if (label.length() > 0) {
            ...
            if (label.length() > 1) {
                labelText = label.substring(2,label.length());
            }
        }

        FontMetrics fm = graphics.getFontMetrics();
        float posX = x - fm.stringWidth(icon) / 2f;
        ...
        if (showBox) {
            ...
            rect.setFrame(posX - outlineSize / 2f,
                    ...
                    fm.stringWidth(labelText) + outlineSize,
        ...
        if (outlineSize > 0) {
            ...
            GlyphVector gv = font.createGlyphVector(frc, labelText);
        ...
        if (null == outlineGlyph) {
            graphics.drawString(icon, posX, posY);
            //change this to display another label under the node with the text on it
        } else {
            graphics.fill(outlineGlyph);
        }
        
        int iconHeight = fm.getHeight();
        Font textFont = new Font("Verdana", Font.PLAIN, fontSize);
        graphics.setFont(textFont);
        fm = graphics.getFontMetrics();
        float posXText = x - fm.stringWidth(labelText) / 2f;
        graphics.drawString(labelText, posXText, posY+(iconHeight/2f));
