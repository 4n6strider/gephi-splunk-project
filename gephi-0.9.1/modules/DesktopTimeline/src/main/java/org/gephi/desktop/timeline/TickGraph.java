        ...
        for (int i = 0; i <= numberTicks; i++) {
            ...
            if (x > 0) {
                ...
                if (font != null && rank >= 1) {
                    ...
                    label = label.replace(",", "");
                    Date time = new Date(Long.parseLong(label)*1000);
                    SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
                    String timeLabel = sdf.format(time);
                    int xLabel = x - (fontMetrics.stringWidth(timeLabel) / 2);
                    g.drawString(timeLabel, xLabel, (int) (h + fontSize * 1.2));
                }
            }
        }
    }
