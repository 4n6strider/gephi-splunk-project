...
//        g2d.fillRect(0, innerY, innerWidth, innerHeight);
        ...
        // timeline bar graph
        double range = tickGraph.getMax()-tickGraph.getMin();
        double interval = range/37.0;
        double barWidth = (double)innerWidth/37.0;
        double barTimeStart = tickGraph.getMin();
        HashMap<Double, Integer> timeBucketCount = new HashMap<Double, Integer>();
        while (barTimeStart < tickGraph.getMax()) {
            timeBucketCount.put(barTimeStart,0);
            barTimeStart += interval;
        }
        ...
        for (Node n: graphNodes) {
            for (Double timekey : timeBucketCount.keySet()) {
                double barTimeEnd = timekey+interval;
                for (Double timestamp : n.getTimestamps()) {
                    if (timekey<=timestamp && timestamp<barTimeEnd) {
                        timeBucketCount.put(timekey, timeBucketCount.get(timekey)+1);
                    }
                }
            }
        }
        int maxNodes = 0;
        for (Double d : timeBucketCount.keySet()) {
            int count = timeBucketCount.get(d);
            if (maxNodes < count) {
                maxNodes = count;
            }
        }
        g2d.setColor(new Color(101, 166, 55, 150));
        //g2d.fillRect(0, innerY+(innerHeight/4), barwidth, innerHeight-innerHeight/4);
        
        for (Double d : timeBucketCount.keySet()) {
            int barHeight = (int)(((double)timeBucketCount.get(d)/(double)maxNodes)*innerHeight);
            int barX = (int) Math.round(((d-tickGraph.getMin())/interval)*barWidth);
            g2d.fillRect(barX, innerY+(innerHeight-barHeight), (int)(barWidth+1), barHeight);
        }
